...
(function () {
  ...
  var mockData = {
    ...
    _getMockByParams: function (format, params, name, traversePath) {
      ...
      switch (format) {
        ...
        case 2: // 数组
          // 数组元素的类型，很长一段时间都做了控制，只能使用一种类型，用户在页面上无法添加多个参数。
          // 现在需要放开这个限制，这意味着用户可以给数组元素指定不同的类型。
          // 如果数组类型的数据模型有多个参数，就生成多个参数的 mock 数据，否则仍旧采用原来的逻辑，即：取第一个参数，默认生成三组数据
          if (params.length > 1) {
            var result = [];
            params.forEach(function (item) {
              var tempResult = this._getArrayMockByArrEleType(item, 1, traversePath, false);
            ...
            return this._getArrayMockByArrEleType(params[0], 3, traversePath, true);
          }
        case 3: // 字符
        case 4: // 数值
        case 5: // 布尔
        ...
        case 6: // 文件
          return '<File>';
        case 7: // 集合
          return this._getHashMapMockData(params, 3, traversePath);
        default:
          break;
      }
    },
     ...
     * @param {Boolean} [checkIfIsArray] - 是否需要检查返回值为数组
     * @return {Array}  - 数组 mock 数据
     */
    _getArrayMockByArrEleType: function (param, repeatTime, traversePath, checkIfIsArray) {
      var result = null;
      if (param.defaultValue) {
        // 数组有默认值时, 求默认值
        ...
        if (checkIfIsArray && !Array.isArray(result)) {
    ...
    _getHashMapMockData: function (params, repeatTime, traversePath) {
      var result = {};
      var keyParam = params.find(function (param) {
        return param.name === '键';
      });
      var valueParam = params.find(function (param) {
        return param.name === '值';
      });
      ...
      var getDefaultValue = function (param) {
        var dv = param.defaultValue;
        if (!param.isArray && param.type <= 10003) {
          return that._getMockOfSysDatatype({
            id: param.type,
            defaultValue: param.defaultValue,
            genExpression: param.genExpression
          });
      ...
      var definedKey;
      var definedValue;
      if (keyParam.defaultValue) {
        definedKey = getDefaultValue(keyParam);
      } else if (keyParam.genExpression) {
        definedKey = this._evalExp(keyParam.genExpression, Object.assign(keyParam, {_isHashMapKey: true}), this.ds);
      }

      if (valueParam.defaultValue) {
        definedValue = getDefaultValue(valueParam);
      } else if (valueParam.genExpression) {
        if (valueParam.isArray) {
          definedValue = this._getMockByParam(valueParam, []);
        } else {
          definedValue = this._evalExp(valueParam.genExpression, valueParam, this.ds);
        }
      }
      ...
      if (!isEmpty(definedKey)) {
        // 键已定义
        if (Array.isArray(definedKey)) {
          // 使用了repeat方法, 键为数组
          for (var i = 0; i < definedKey.length; i++) {
            var val;
            if (isEmpty(definedValue)) {
              // 值未定义 使用mock
              val = valueParam.isArray ?
                this._getMockByParam(valueParam, []) : this._getMockOfDatatypeId(valueParam.type, valueParam.name, traversePath);
            } else {
              // 值已定义
              val = definedValue;
            }
            result[definedKey[i]] = val;
          }
        } else {
          // 键不为数组
          var val;
          ...
          result[definedKey] = val;
        }
      } else {
        var pathNodeNum = null;
        for (var i = 0; i < repeatTime; i++) {
          pathNodeNum = traversePath.length;
          var keyMock = this._getMockOfDatatypeId(keyParam.type, keyParam.name, traversePath);
          var valueMock;
          if (!isEmpty(definedValue)) {
            valueMock = definedValue;
          } else {
            if (valueParam.isArray) {
              valueMock = this._getMockByParam(valueParam, []);
            } else {
              valueMock = this._getMockOfDatatypeId(valueParam.type, valueParam.name, traversePath);
            }
          }
          result[keyMock] = valueMock;
          traversePath.length = pathNodeNum;
        }
      }
  ...
  var publicRuleMethods = {
    ...
    repeat: function (num, arrEleGenFunc) {
      ...
      if (datatype && datatype.format === 7) {
        // 集合类型
        num = isNaN(num) ? 3 : num <= 0 ? 3 : num;
        return mockData._getHashMapMockData(datatype.params, num, []);
      } else if (param._isHashMapKey) {
        num = isNaN(num) ? 3 : num <= 0 ? 3 : num;
        var rt = [];
        for (var i = 0; i < num; i++) {
          rt.push(mockData._getMockOfSysDatatype(datatype));
        }
        return rt;
      } else if (param._isArray) {
        // _isArray 表示是数组
        var arr = [];
        num = isNaN(num) ? 3 : num <= 0 ? 3 : num;
        ...
        var genArr = function () {
          ...
          if (datatype.format == 0) {
            ...
            return mockData._getArrayMockByArrEleType(_param, num, [], true);
          } else {
            var _isArray = datatype.format === 2;
            if (datatype.params.length <= 1) {
              _param = {
                isArray: _isArray,
              ...
              for (var i = 0; i < num; i++) {
                // 随机取一个
                var tempParam = datatype.params[Math.floor((Math.random() * datatype.params.length))];
                _param = {
                  isArray: tempParam.isArray,
                  type: tempParam.type,
                  name: tempParam.typeName
                };
                var tempResult = mockData._getArrayMockByArrEleType(_param, 1, [], true);
                result.push(tempResult[0]);
              }
              return [result];
            }
        ...
        return '提示: NEI.repeat 方法只适用于数组类型、集合类型与集合键';
      }
    },
    /**
