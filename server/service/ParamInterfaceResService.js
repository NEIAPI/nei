...
class ParamInterfaceResService extends require('./ParamInterfaceService') {
  constructor(uid, context) {
    super(uid, context, '../dao/ParamInterfaceResDao');
  }
  ...
  * _afterUpdate({parentId, newParam, oldParam}) {
    ...
    if (isNameUpdated || isTypeUpdated) {
      ...
      const itf = yield interfaceService.findDetailById(interfaceId);
      ...
      if (itf.resFormat === db.MDL_FMT_HASHMAP) {
        ...
        if (isNameUpdated) {
          ...
          if (resourceUtil.isCustomDatatype(newParam.type)) {
            ...
            constraints = yield interfaceService.getConstraints({projectIds: itf.projectId});
          }
          yield mockStoreService.updateParamNameOrTypeByKeysPathsForInterfaces({
            itfArr: [itf],
            keysPathsMap: {[itf.id]: [keysPath]},
    ...
    if (isNameUpdated || isTypeUpdated || isIgnoredUpdated || isRequired || isGenExpressionUpdated) {
  ...
  * _afterCreate({parentId, createdParams}) {
    ...
    const constraints = yield interfaceService.getConstraints({projectIds: itf.projectId});
...
module.exports = ParamInterfaceResService;
