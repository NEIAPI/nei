...
NEJ.define([
  ...
  _pro.__doBuild = function () {
    ...
    this.rpcRenderNode = e._$getByClassName(this.__body, 'markdown-body-doc')[0];
  ...
  _pro.__onRefresh = function (_options) {
    ...
    this.rpcRenderNode.innerHTML = util._$renderRpcsByRoute(this.route);
    ...
    var rpcData = this.cache._$getRpcData();
    u._$createSampleCode(this.rpcRenderNode, 'rpcs', rpcData, constraintsData, datatypeData);
    ...
    var ic = caches.rpc._$allocate();
    var rpcListKey = 'rpc-' + projectId;
    ic._$setListInCache(rpcListKey, rpcData);
    // 设置数据模型缓存
    ...
    if (this.route.id) {
      ...
      for (var i = 0; i < ids.length; i++) {
        var paramContainer = e._$get('m-rpc-inputs-' + ids[i]);
        if (paramContainer) {
          paramEditor._$$ParamEditor._$allocate({
            parent: paramContainer,
            parentId: ids[i],
            parentType: db.PAM_TYP_RPC_INPUT,
            pid: projectId,
            preview: true,
            forceReadonly: true,
            docPreview: true
      ...
      for (var i = 0; i < ids.length; i++) {
        var paramContainer = e._$get('m-rpc-outputs-' + ids[i]);
        if (paramContainer) {
          paramEditor._$$ParamEditor._$allocate({
            parent: paramContainer,
            // 接口id
            parentId: ids[i],
            parentType: db.PAM_TYP_RPC_OUTPUT,
            pid: projectId,
            preview: true,
            forceReadonly: true,
            docPreview: true
