...
NEJ.define([
  ...
  _p._$$CacheRpc = _k._$klass();
  pro = _p._$$CacheRpc._$extend(_d._$$Cache);
  _p._$cacheKey = 'rpc';
  _p._$cacheRefKey = 'rpc-ref-';
  _p._$cacheBatKey = 'rpc-bat';
   ...
   * @property {String} options.path -  RPC 接口请求地址
   * @property {String} options.method -  RPC 接口请求方式
  ...
  pro.__doUpdateItem = function (options) {
    ...
    options.onload = function (result) {
      if (result.params) {
        ...
        var rpcData = this._$getItemInCache(result.id);
        Object.keys(rpcData.params).forEach(function (item) {
          if (!result.params.hasOwnProperty(item)) {
            result.params[item] = rpcData.params[item];
          }
        });
      }
  ...
  pro._$deleteItems = function (options) {
    options.onload = function (event) {
      // 如果删除的是公共资源库中的 RPC 接口, 则其他项目(同个项目组)中对应的 RPC 接口也删除
      var _proCache = proCache._$$CachePro._$allocate();
      ...
      deletedList.forEach(function (item) {
        ...
        var allRpcs = this.__getHash();
        Object.keys(allRpcs).forEach(function (rpcId) {
          var rpc = allRpcs[rpcId];
          if (rpc.versions && rpc.versions.length) {
            rpc.versions = rpc.versions.filter(function (it) {
    ...
    this.__doAction({
      ...
      onload: function (evt) {
        ...
        var sharedItem = list.find(function (item) {
          return item.id === evt.data.rpc.id;
        });
        // 分享后, 它的 projectId 有变化了, 需要更新
        Object.assign(sharedItem, evt.data.rpc);
        // 设置分享字段
  ...
  /**
   * 从服务器获取引用某个 RPC 接口的页面列表
   * @param {Object} options - 参数
   * @property {Number} options.id -  RPC 接口的id
   ...
   * 批量获取接口详情
   * @param {Object} options - 参数
   * @property {String} options.ids -  RPC 接口的 id 列表, 以逗号分隔
   ...
   * @property {Array} options.data.copys 复制的 RPC 接口数据
   ...
   * @property {Number} options.data.pid 项目id
   * @property {Number} options.data.gid 分组id
   * @property {Array} options.data.moves 移动的 RPC 接口数据
   * @property {String} options.data.tags 标签
   */
   ...
   * @property {Array} options.data.items.rpcs 接口数据
   * @property {String} options.data.items.rpcs.name 接口名称
   * @property {String} options.data.items.rpcs.method 接口请求方法
   * @property {String} options.data.items.rpc.path 接口路径
   * @private
   */
    ...
    this.__doAction({
      ...
      onload: function (result) {
        ...
        Object.keys(hash).forEach(function (iid) {
          ...
          if ((rpc.version &&
            rpc.version.origin &&
            rpc.version.origin === inter.version.origin) ||
            rpc.id === inter.version.origin) {
            rpc.versions = rpc.versions || [];
            rpc.versions.unshift(inter);
          }
          if (rpc.id === inter.version.origin && !rpc.version) {
            rpc.version = {
              parent: parseInt(iid),
              origin: parseInt(iid),
              name: '初始版本'
  ...
  pro._$audit = function (options) {
    ...
    this.__doAction({
      ...
      onload: function (data) {
        options.onload(data);
        var rpcData = this._$getItemInCache(data.data.id);
        rpcData.status = data.data.status;
        rpcData.statusId = data.data.statusId;
      }._$bind(this),
      method: 'PUT',
      action: 'audit'
    });
   ...
   * @property {Number} options.data.groupId 分组id
   * @property {Array} options.data.items 导入的 RPC 接口
   ...
   * @return {Array} - 类名列表
   */
  pro._$getClassNameList = function (pid) {
    var listKey = this._$getListKey(pid);
    var list = this._$getListInCache(listKey);
    var result = {};
    var classNameArr = [];
    list.forEach(function (item) {
      if (!result[item.className]) {
        result[item.className] = true;
        classNameArr.push({
          id: item.className,
          name: item.className
        });
